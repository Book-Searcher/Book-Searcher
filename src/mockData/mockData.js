export default {
  allBooks: [
    {
      volumeInfo: {
        title: 'Hermeneutics',
        authors: ['John D. Caputo'],
        publishedDate: '2018-01-25T00:00:00.000Z',
        description:
          "Is anything ever not an interpretation? Does interpretation go all the way down? Is there such a thing as a pure fact that is interpretation-free? If not, how are we supposed to know what to think and do? These tantalizing questions are tackled by renowned American thinker John D Caputo in this wide-reaching exploration of what the traditional term 'hermeneutics' can mean in a postmodern, twenty-first century world. As a contemporary of Derrida's and longstanding champion of rethinking the disciplines of theology and philosophy, for decades Caputo has been forming alliances across disciplines and drawing in readers with his compelling approach to what he calls \"radical hermeneutics.\" In this new introduction, drawing upon a range of thinkers from Heidegger to the Parisian \"1968ers\" and beyond, he raises a series of probing questions about the challenges of life in the postmodern and maybe soon to be 'post-human' world.'",
        categories: ['Philosophy'],
        language: 'un',
        pageCount: '280',
        imageLinks: {
          thumbnail:
            'http://books.google.com/books/content?id=G2wwDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
        },
        link:
          'http://books.google.com.ua/books?id=G2wwDwAAQBAJ&printsec=frontcover&dq=age&hl=&cd=17&source=gbs_api',
      },
      saleInfo: {
        buyLink:
          'https://play.google.com/store/books/details?id=G2wwDwAAQBAJ&rdid=book-G2wwDwAAQBAJ&rdot=1&source=gbs_api',
        listPrice: {
          amount: '345.09',
          currencyCode: 'UAH',
        },
      },
    },
    {
      _id: '60b3b9cab406ba30f444ac76',
      volumeInfo: {
        title: 'Hermeneutics',
        authors: ['John D. Caputo'],
        publishedDate: '2018-01-25T00:00:00.000Z',
        description:
          "Is anything ever not an interpretation? Does interpretation go all the way down? Is there such a thing as a pure fact that is interpretation-free? If not, how are we supposed to know what to think and do? These tantalizing questions are tackled by renowned American thinker John D Caputo in this wide-reaching exploration of what the traditional term 'hermeneutics' can mean in a postmodern, twenty-first century world. As a contemporary of Derrida's and longstanding champion of rethinking the disciplines of theology and philosophy, for decades Caputo has been forming alliances across disciplines and drawing in readers with his compelling approach to what he calls \"radical hermeneutics.\" In this new introduction, drawing upon a range of thinkers from Heidegger to the Parisian \"1968ers\" and beyond, he raises a series of probing questions about the challenges of life in the postmodern and maybe soon to be 'post-human' world.'",
        categories: ['Philosophy'],
        language: 'un',
        pageCount: '280',
        imageLinks: {
          thumbnail:
            'http://books.google.com/books/content?id=G2wwDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
        },
        link:
          'http://books.google.com.ua/books?id=G2wwDwAAQBAJ&printsec=frontcover&dq=age&hl=&cd=17&source=gbs_api',
      },
      saleInfo: {
        buyLink:
          'https://play.google.com/store/books/details?id=G2wwDwAAQBAJ&rdid=book-G2wwDwAAQBAJ&rdot=1&source=gbs_api',
        listPrice: {
          amount: '345.09',
          currencyCode: 'UAH',
        },
      },
    },
  ],
  bookFindDistinctResult: [
    {
      _id: '6091bb23d631ee088c07eaf4',
      title: 'Dream Finder',
      authors: 'Roger Taylor',
      publishedDate: '2002-01-01T00:00:00.000Z',
      description: 'Since their leader Petran died, the Guild of',
      categories: 'Fiction',
      language: 'en',
      img:
        'http://books.google.com/books/content?id=E5K5pRh3xtUC&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
      link:
        'http://books.google.com.ua/books?id=E5K5pRh3xtUC&pg=PA801&dq=rendd&hl=&cd=3&source=gbs_api',
      amount: 'Not for sale',
      currencyCode: '',
      __v: 0,
    },
    {
      _id: '6091bc149852524fa80e55ec',
      title: 'Лес шумит',
      authors: 'Владимир Короленко',
      publishedDate: '2018-02-09T00:00:00.000Z',
      description:
        '«Золотой фонд мировой литературы» - коллекция электронных книг, включающая лучшие образцы мировой художественной литературы, представляет собой максимально исчерпывающий список самых читаемых книг мира. Каждое из произведений, изданных под обложкой этой серии, входит в один или сразу несколько списков лучших книг по разным версиям, которые не противопоставляются один другим, а гармонично объединяются, чтобы предоставить читателю наибольший выбор. «Лес шумит» — захватывающий рассказ великого русского и украинского писателя Владимира Галактионовича Короленко (1853 — 1921).*** Охотник едет по лесу и останавливается передохнуть возле маленькой избушки. Хозяин избы, старый дед, рассказывает путнику древнюю полесскую легенду о лесном хозяине. Другими известными произведениями Владимира Короленко являются «Чудная», «Эпизоды из жизни "Искателя"», «Яшка», «Сон Макара», «Без языка», «Дети подземелья», «Соколинец», «За иконой». Владимир Галактионович Короленко был необычайно популярен уже при жизни, им восхищался М. Горький, Л. Толстой, А. Чехов. Кроме того, он вел активную общественную деятельность, его называли «совестью России». В Украине существует литературная премия имени Короленко за лучшее русскоязычное произведение среди украинских литераторов.',
      categories: 'Fiction',
      language: 'ru',
      img:
        'http://books.google.com/books/content?id=ejtLDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
      link:
        'http://books.google.com.ua/books?id=ejtLDwAAQBAJ&pg=PT29&dq=%D0%BB%D0%B5%D1%81&hl=&cd=2&source=gbs_api',
      buyLink:
        'https://play.google.com/store/books/details?id=ejtLDwAAQBAJ&rdid=book-ejtLDwAAQBAJ&rdot=1&source=gbs_api',
      amount: '1.09',
      currencyCode: 'UAH',
      __v: 0,
    },
    {
      _id: '6091bafbd631ee088c07eaf0',
      title: 'The Papers of Thomas Jefferson, Volume 37',
      authors: 'Thomas Jefferson',
      publishedDate: '2018-06-05T00:00:00.000Z',
      description:
        'This volume opens on 4 March 1802, the first anniversary of Thomas Jefferson\'s inauguration as the nation\'s third president, and closes on 30 June. In March, a delegation of Seneca Indians comes to Washington to discuss their tribe\'s concerns, and Jefferson names a commissioner to handle a land sale by Oneida Indians to the state of New York. In April, the Senate ratifies a treaty with the Choctaw nation for a wagon road across their lands. Jefferson worries about an increasingly dictatorial France taking back control of New Orleans, prompting him to the intemperate remark that he would "marry" America\'s fortunes to the British fleet. Charles Willson Peale sends him sketches of the skull of a prehistoric bison found in Kentucky. During the closing, and very frustrating, weeks of Congress, he distracts himself with a cipher devised by Robert Patterson. He prepares lists of books to be purchased for the recently established Library of Congress and also obtains many titles for his own collection. Even while he is in Washington occupied with matters of state, Jefferson has been keeping close watch on the renovations at Monticello. In May, he has Antonio Giannini plant several varieties of grapes in the southwest vineyard, and he orders groceries, molasses, dry Lisbon wine, and cider to be shipped to Monticello in time for his arrival. He looks forward "with impatience" to the moment he can embrace his family once more.',
      categories: 'History',
      language: 'en',
      pageCount: 824,
      img:
        'http://books.google.com/books/content?id=If5ZDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
      link:
        'http://books.google.com.ua/books?id=If5ZDwAAQBAJ&pg=PA634&dq=rendd&hl=&cd=2&source=gbs_api',
      buyLink:
        'https://play.google.com/store/books/details?id=If5ZDwAAQBAJ&rdid=book-If5ZDwAAQBAJ&rdot=1&source=gbs_api',
      amount: '4227.26',
      currencyCode: 'UAH',
      __v: 0,
    },
  ],
  bookFindOneResult: {
    _id: '6091bb23d631ee088c07eaf5',
    title: 'Dream Finder',
    authors: 'Roger Taylor',
    publishedDate: '2002-01-01T00:00:00.000Z',
    description: 'Since their leader Petran died, the Guild of',
    categories: 'Fiction',
    language: 'en',
    img:
      'http://books.google.com/books/content?id=E5K5pRh3xtUC&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
    link:
      'http://books.google.com.ua/books?id=E5K5pRh3xtUC&pg=PA801&dq=rendd&hl=&cd=3&source=gbs_api',
    amount: 'Not for sale',
    currencyCode: '',
    __v: 0,
  },
  listFindDistinctResult: {
    _id: '608f07de49e8f2433491500c',
    owner: '608eac402c5694130058b0df',
    type: 'wantToReadList',
    books: [
      { _id: '6091bb23d631ee088c07eaf5', book: '6091bb23d631ee088c07eaf4' },
      { _id: '6091bc149852524fa80e55ed', book: '6091bc149852524fa80e55ec' },
      { _id: '6091be546ea1712e8c0a6ea8', book: '6091bafbd631ee088c07eaf0' },
    ],
    __v: 8,
  },
  listResponse: {
    _id: '608f07de49e8f2433491500c',
    owner: '608eac402c5694130058b0df',
    type: 'favList',
    books: [
      { _id: '6091bb23d631ee088c07eaf5', book: '6091bb23d631ee088c07eaf4' },
      { _id: '6091bc149852524fa80e55ed', book: '6091bc149852524fa80e55ec' },
      { _id: '6091be546ea1712e8c0a6ea8', book: '6091bafbd631ee088c07eaf0' },
    ],
    __v: 8,
  },
  bookInfo: {
    title: 'Hermeneutics',
    publishedDate: '2018-01-25T00:00:00.000Z',
    description:
      "Is anything ever not an interpretation? Does interpretation go all the way down? Is there such a thing as a pure fact that is interpretation-free? If not, how are we supposed to know what to think and do? These tantalizing questions are tackled by renowned American thinker John D Caputo in this wide-reaching exploration of what the traditional term 'hermeneutics' can mean in a postmodern, twenty-first century world. As a contemporary of Derrida's and longstanding champion of rethinking the disciplines of theology and philosophy, for decades Caputo has been forming alliances across disciplines and drawing in readers with his compelling approach to what he calls \"radical hermeneutics.\" In this new introduction, drawing upon a range of thinkers from Heidegger to the Parisian \"1968ers\" and beyond, he raises a series of probing questions about the challenges of life in the postmodern and maybe soon to be 'post-human' world.'",
    language: 'un',
    pageCount: '280',
    img:
      'http://books.google.com/books/content?id=G2wwDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
    link: '',
    authors: 'John D. Caputo',
    categories: 'Philosophy',
    buyLink:
      'https://play.google.com/store/books/details?id=G2wwDwAAQBAJ&rdid=book-G2wwDwAAQBAJ&rdot=1&source=gbs_api',
    amount: '345.09',
    currencyCode: 'UAH',
  },
  bookInfoWithId: {
    _id: '60b3b9cab406ba30f444ac76',
    title: 'Hermeneutics',
    publishedDate: '2018-01-25T00:00:00.000Z',
    description:
      "Is anything ever not an interpretation? Does interpretation go all the way down? Is there such a thing as a pure fact that is interpretation-free? If not, how are we supposed to know what to think and do? These tantalizing questions are tackled by renowned American thinker John D Caputo in this wide-reaching exploration of what the traditional term 'hermeneutics' can mean in a postmodern, twenty-first century world. As a contemporary of Derrida's and longstanding champion of rethinking the disciplines of theology and philosophy, for decades Caputo has been forming alliances across disciplines and drawing in readers with his compelling approach to what he calls \"radical hermeneutics.\" In this new introduction, drawing upon a range of thinkers from Heidegger to the Parisian \"1968ers\" and beyond, he raises a series of probing questions about the challenges of life in the postmodern and maybe soon to be 'post-human' world.'",
    language: 'un',
    pageCount: '280',
    img:
      'http://books.google.com/books/content?id=G2wwDwAAQBAJ&printsec=frontcover&img=1&zoom=1&edge=curl&source=gbs_api',
    link: '',
    authors: 'John D. Caputo',
    categories: 'Philosophy',
    buyLink:
      'https://play.google.com/store/books/details?id=G2wwDwAAQBAJ&rdid=book-G2wwDwAAQBAJ&rdot=1&source=gbs_api',
    amount: '345.09',
    currencyCode: 'UAH',
  },
};
